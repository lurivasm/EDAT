...
struct index_ {
    FILE *fich; /*Fichero*/
    char *path; /*Nombre del fichero*/
    int num;    /*Número de filas*/
    long pos;   /*Posición de memoria del primer puntero*/
    long *next  /*Puntero al siguiente (suele ser NULL para agregar uno nuevo)*/
    int key;
};
...
index_t* index_open(char* path) {
  ...
  i = (index_t*)malloc(sizeof(index_t));
  if(!i) return NULL;
  ...
  strcpy(i->path, path);
  i->num = 0;
  fread(&i->type, sizeof(int), 1, i->fich);
  i->pos = ftell(i->fich);
